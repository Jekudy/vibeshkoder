...
| R8.b | Refuse to plan when the evidence bundle is abstained (empty). Abstention
         pattern — never a hallucinated empty-citation plan. |
...
R8.b uses the live ``db_session`` so the real rate-bucket + budget queries run.
...
_counter = itertools.count(80_000)
...
# ---------------------------------------------------------------------------
# Shared fakes (mirror tests/services/test_butler_undo.py shapes)
# ---------------------------------------------------------------------------


def _bundle(*, abstained: bool, mvids: tuple[int, ...] = (10, 11)) -> EvidenceBundle:
    items = tuple(
        EvidenceItem(
            message_version_id=m,
            chat_message_id=m + 1000,
            chat_id=-100_500,
            message_id=m + 2000,
            user_id=55,
            snippet="s",
        ...
        for m in (() if abstained else mvids)
    )
...
# ---------------------------------------------------------------------------
# R8.b — abstain on empty evidence (DB-backed: real rate buckets + budget query)
# ---------------------------------------------------------------------------


async def test_r8b_refuses_plan_on_empty_evidence(db_session) -> None:
    """R8.b: an abstained (empty) bundle refuses BEFORE the LLM gateway is called."""
    ...
    svc = ButlerService(
        ...
        user_repo=None,  # skip membership — exercising the abstain branch
        llm_gateway=_ExplodingGateway(),
        evidence_builder=_FakeEvidenceBuilder(_context(abstained=True, requester=requester)),
        settings=_Settings(),
    )
    ...
    assert ei.value.error_kind == "empty_evidence"

    # An audit row was written (status='rejected', reason='empty_evidence').
    row = await ButlerActionRepo.get(db_session, ei.value.action_id)
    assert row is not None
    assert row.status == "rejected"
    assert row.rejection_reason == "empty_evidence"
    assert row.llm_usage_ledger_id is None  # no LLM budget spent
