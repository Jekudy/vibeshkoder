...
**Last updated:** 2026-05-27 (T12-08 merged + T12-07 docs gate — /butler_undo + 5 rollback kinds + migrations 077/078, 115 tests green, PR #351 pending merge.)
...
| T12-07 | 2 | `/butler_undo` full implementation + 5 rollback kinds + audit table + TTL + idempotency. Migration 077: `butler_undo_invocations` table + widens `ck_butler_actions_status` CHECK to add `'undone'`. `ButlerUndoInvocation` ORM + `ButlerUndoInvocationRepo` (flush-only, UNIQUE `(butler_action_id, butler_tool_invocation_id)`). `ButlerCardSuggestionRepo.dismiss_by_undo` rejects linked EC (pending→rejected) for `cancel_pending` path. Forget cascade tail: `butler_undo_invocations` layer inserted between `butler_tool_invocations` and `butler_actions` in `CASCADE_LAYER_ORDER`; `_cascade_butler_undo_invocations` redacts `error_message`. `ButlerService.execute_undo`: `FOR UPDATE NOWAIT` → auth → status gate (`succeeded`/`undone`) → TTL (`BUTLER_UNDO_TTL_MINUTES` env, default 60 min) → idempotency check → LIFO over invocations → dispatch 5 rollback kinds → set action `status='undone'`. Handler replaces T12-05 stub: dual flag gate (`memory.butler.enabled` + `memory.butler.undo.enabled` both default OFF), error routing for all `ButlerActionError` kinds + `CascadeInFlightError`, HTML step summary. | PR pending | Branch `feat/p12-t12-07-undo`. Commits `6baa506`..`e666674`. Alembic head 076 → 077. 105 passed, 6 skipped (combined suite). Privacy lint exit 0. Ruff exit 0. Phase 11 binding **86 → 86** (L12/C10/I9/R8/G3 lands in T12-09). |
