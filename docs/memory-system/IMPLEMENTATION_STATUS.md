...
## Phase 12 — Butler / action execution (CLOSED 2026-05-30)
...
| T12-02 | 1 | `ButlerEvidenceContext` + `butler_context_hash` + `build_butler_evidence` (spec §3.6 / §4.2) | merged | In main via Phase 12 wave merges (closure 2026-05-30); branch deleted. 43 unit + 9 L11 binding tests green. Phase 11 binding 77 → 86. No flag flipped; foundation for T12-04 confirm_action + T12-08 hash replay. Tool wrapper deferred to T12-06. |
| T12-03 | 1 | Butler tools registry + LLM gateway entry points: `ButlerTool` Protocol (§5.C), `ButlerPlan` pydantic v2 + `ALLOWED_BUTLER_TOOLS` frozenset (5 names), `validate_butler_plan`, `plan_butler_action` (`call_type='butler_decision'`) + `synthesize_butler_summary` (`call_type='butler_summary'`) in `llm_gateway.py` | merged | In main via Phase 12 wave merges (closure 2026-05-30); branch deleted. Commits `b17509f`..`7de6e18`. 77 unit + gateway tests green (44 registry + 26 plan + 7 synthesize). Phase 11 binding **86 → 86** (T12-03 adds no binding tests; L12/C10/I9/R8/G3 family lands in T12-09). No flag flipped; foundation for T12-04 confirm_action + T12-06 tool wrappers + T12-08 hash replay. |
| T12-04 | 2 | `ButlerService` state machine (`plan_action`/`confirm_action`/`execute_action`/`cancel_action`/`expire_action`) + 6 exception classes + MSK rate buckets w/ rollback + cross-user consent re-verified at execute + cascade-vs-callback `SELECT FOR UPDATE NOWAIT` guard. Migration 074 adds `query`/`visibility_scope`/`plan_payload` to `butler_actions` + `confirmation_token`+UNIQUE on `butler_action_confirmations`. ORM + 4 repos extended. `forget_cascade._cascade_butler_action_confirmations` widened for parent-row lock. | merged | In main via Phase 12 wave merges (closure 2026-05-30); branch deleted. Commits `3a09ecb`..`d70bb39` (incl. deep review fix cycle: 4 CRITICAL + 4 HIGH + class-identity + alembic head + rate-bucket rollback completeness). Alembic head 073 → 074. 58 state-machine + 33 repo + 9 cascade tests green (100 new). Phase 11 binding **86 → 86** (T12-04 adds no binding tests; L12/C10/I9/R8/G3 lands in T12-09). No flag flipped; foundation for T12-05 handlers + T12-06 tool wrappers + T12-07 undo + T12-08 reaper. |
| T12-05 | 2 | `bot/handlers/butler.py` — Telegram handlers: `/butler`, `/butler_status`, `/butler_cancel`, `/butler_undo` (stub) + 4 inline keyboard callbacks (confirm/cancel/affected_approve/affected_reject). DM-only (PrivateChatFilter), member auth gate, master flag `memory.butler.enabled` (default OFF) + 5 per-tool flags. Cross-user consent DM with Approve/Reject keyboard. 8 §5.B rejection paths via `_dispatch_butler_error`. `ButlerService.revoke_affected_user_consent` + `AffectedUserUnreachableError` (additive). Migration 075 widens `ck_butler_action_confirmations_status` to include `'revoked'`. HTML-escaping in `_render_preview`. Round-2 fix: all early-return paths (flag-OFF + per-tool disabled) call `await session.rollback()` before return to defeat `DbSessionMiddleware` unconditional commit; tests assert `rollback.assert_awaited_once()`. | merged | PR #348 merged 2026-05-27. Branch `feat/p12-t12-05-handlers`. Commits `d7045ca`..`4c09fe5` (2 rounds: 1C+2H+7M+4L round-1; H-NEW-1 session lifecycle round-2). Alembic head 074 → 075. 86 tests green (+9 vs initial; includes `test_affected_user_reject_real_service_path` with REAL `ButlerService`). Privacy lint exit 0. Phase 11 binding **86 → 86** (L12/C10/I9/R8/G3 family lands in T12-09). |
| T12-06 | 2 | 5 butler tool implementations under `bot/services/butler_tools/`: `recall_evidence` (delegates to sealed `ButlerEvidenceContext`, no DB reads, rollback_kind=`not_reversible`), `schedule_meeting` (Telegram-native proposal via `bot.send_message`, rollback_kind=`delete_message`), `send_intro` (confirmed text from args, no re-fetch, rollback_kind=`delete_message`), `update_intro` (ownership check via `invocation_repo.find_by_posted_message_id` + edit-or-followup fallback, rollback_kind=`edit_message`|`followup_correction`), `suggest_card_creation` (writes `extraction_candidates`+`butler_card_suggestions`, never approved, rollback_kind=`cancel_pending`, `action_id` required). `TOOL_DISPATCH` registry in `bot/services/butler_tools/__init__.py` with module-load assertion. `ButlerTool` Protocol aligned with real tool signatures `(ctx, args, *, session, bot, action_repo, invocation_repo, action_id)`. `ButlerService.execute_action` validates `step_args` via `TOOL_ARGS_SCHEMA.model_validate`, threads bot/action_repo/invocation_repo/action_id, invokes `tool.build_inverse(result)`. Migration 076 adds `butler_tool_invocations.posted_message_id` BIGINT NULLABLE + partial index + `find_by_posted_message_id` repo method; `update_invocation` accepts `posted_message_id` kwarg, butler.py populates for send_intro/schedule_meeting. Narrow `except (SQLAlchemyError, OperationalError)` in `update_intro` (was blanket `except Exception`). All 15 `getattr(args, X, default)` reintroductions removed (each tool raises `invariant_broken`). `parse_mode=None` pinned on send/edit/followup. | merged (#349, 2026-05-27) | Branch `feat/p12-t12-06-tools` (deleted). Commits `a411e9e`..`3122431` (2 rounds: 3C+2H+3M round-1; C2 dual-stage wiring + HIGH-NEW-1 round-2). Alembic head 075 → 076. 136 tests green (+7; includes `test_butler_execute_action_e2e.py` integration with real `RecallEvidenceTool` and `test_update_intro_finds_owned_message_e2e` regression guard). Privacy lint exit 0. Phase 11 binding **86 → 86** (L12/C10/I9/R8/G3 family lands in T12-09). 2-round dual-model review (Claude product ACCEPTED, Claude tech APPROVE). |
| T12-07 | 2 | `/butler_undo` full implementation + 5 rollback kinds + audit table + TTL + idempotency. Migration 077: `butler_undo_invocations` table + widens `ck_butler_actions_status` CHECK to add `'undone'`. Migration 078: `butler_tool_invocations.inverse_op_payload` JSONB column — stores per-invocation inverse payload populated by `execute_action` after `tool.build_inverse(result)`. `ButlerUndoInvocation` ORM + `ButlerUndoInvocationRepo` (flush-only, UNIQUE `(butler_action_id, butler_tool_invocation_id)`). `ButlerCardSuggestionRepo.dismiss_by_undo` rejects linked EC (pending→rejected), sets `reviewed_by`+`reviewed_at` (CHECK constraint compliance). `_resolve_prior_text` helper queries `message_versions` JOIN `chat_messages` for second-latest pre-edit text; cascade-aware (NULL on redaction → returns None); narrow `except (SQLAlchemyError, OperationalError)`. Forget cascade tail: `butler_undo_invocations` layer inserted between `butler_tool_invocations` and `butler_actions` in `CASCADE_LAYER_ORDER`; `_cascade_butler_undo_invocations` redacts `error_message`. `ButlerService.execute_undo`: `FOR UPDATE NOWAIT` → auth → status gate (`succeeded`/`undone`) → idempotency (BEFORE TTL) → TTL (`BUTLER_UNDO_TTL_MINUTES` env, default 60 min) → LIFO over invocations → dispatch 5 rollback kinds → set action `status='undone'`. Constructor wiring: `butler_undo_invocation_repo` + `butler_card_suggestion_repo` via kwargs (no `getattr` defaults). `send_intro.build_inverse` emits `chat_id` (was `target_user_id`). Handler replaces T12-05 stub: dual flag gate (`memory.butler.enabled` + `memory.butler.undo.enabled` both default OFF), coded error keys (no raw exc text), HTML step summary. 10 real-producer integration tests in `tests/integration/test_butler_undo_e2e.py` + 3 `_resolve_prior_text` tests (happy/no-prior/SQLAlchemyError). | merged (#351, 2026-05-30) | Branch `feat/p12-t12-07-undo` (deleted). Commits `a930208`..`0ea9656` (3 rounds: round-1 — 4C+3H+5M; round-2 — all round-1 closed + 1 CRITICAL REGRESSION introduced: `_resolve_prior_text` SQL using non-existent ORM columns, masked by mock-only tests; round-3 — JOIN through `chat_messages`, narrow except, 3 new tests). Alembic head 076 → 077 → 078. 115 tests green (+13 across rounds: 10 e2e + 3 resolve_prior_text). Privacy lint exit 0. Ruff exit 0. Phase 11 binding **86 → 86** (L12/C10/I9/R8/G3 lands in T12-09). 3-round dual-model review: Claude product ACCEPTED + Claude tech APPROVE (Codex companion stalled; used the documented independent-review fallback). |

| T12-08 | 3 | Butler TTL expiry worker + per-user-day budget cap + rate bucket exhaustiveness + `monthly_cost_usd` call_type extension. `bot/services/butler_budget.py`: `ButlerBudgetChecker` (per-user-day ceiling via `BUTLER_PER_USER_DAILY_USD_CEILING` default $0.20) + `compute_user_daily_budget_spent` (JOIN `llm_usage_ledger ↔ butler_actions` for MSK-day SUM). `bot/services/scheduler.py`: `butler_expire_tick_job` APScheduler wrapper gated by `memory.butler.enabled`, `args=[bot]` (T7 FHR F2 pattern); TTL tick uses `_expire_action_inline` free function (M1 fix — avoids `ButlerService(None,...)` antipattern). `expire_action` uses `FOR UPDATE NOWAIT` (M2 fix). `bot/db/repos/butler_action.py`: `get_pending_past_ttl` with `.limit(BUTLER_EXPIRE_BATCH_SIZE)` default 200 (H2 fix). `bot/db/repos/llm_usage_ledger.py`: `monthly_cost_usd` gains `call_type: str | None = None` kwarg; `LedgerRepoProtocol` signature updated to match (H3 fix). `llm_gateway._butler_budget_check` monthly cap now sums `call_type='butler_decision'` + `call_type='butler_summary'` (H1 fix, TODO removed). `ButlerBudgetChecker` wired into `ButlerService.plan_action` between rate-bucket increments and evidence build; on budget exceeded → rate buckets rolled back + audit row + raises `ButlerActionError(error_kind='budget_exceeded')` (C1 fix). `.env.example` / `.env.dev.example` / `.env.production.example` document 3 new vars: `BUTLER_EXPIRE_TICK_SECONDS=60`, `BUTLER_EXPIRE_BATCH_SIZE=200`, `BUTLER_PER_USER_DAILY_USD_CEILING=0.20` (M3 fix). No migration needed (uses existing `butler_actions.expires_at` from T12-04 + existing `llm_usage_ledger`). | merged (#350, 2026-05-27) | Branch `feat/p12-t12-08-controls` (deleted). Commits `e8dc08f`..`92326c3` (round 1: initial impl; round 2: C1 budget wiring + H1 monthly filter + H2 LIMIT + H3 Protocol kwarg + M1 free fn + M2 FOR UPDATE NOWAIT + M3 .env vars + M4 configurable fake session). No new migration. 80 tests green (+6 vs round-1 from C1 budget integration + Protocol assertions; 7 skipped = DB-backed without Postgres). Privacy lint exit 0. Phase 11 binding **86 → 86** (delta 0; L12/C10/I9/R8/G3 lands in T12-09). No flag flipped. 2-round dual-model review: Claude product ACCEPTED + Claude tech APPROVE (Codex companion stalled; used the documented independent-review fallback). |

| T12-09 | 3 | Phase 11 binding suite extension 77 → 102 (+25 ACs: L11.a-e, C10.a-c, I9.a-f, R8.a-g, G3.a-d) across `tests/evals/test_butler_{leakage,citations,forget_cascade,refusal,drift}.py` + `test_no_llm_imports.py` (G3.a append). Runtime guard: `ButlerService.plan_action` Step 2b rejects when `evidence_context.bundle.abstained` is True → `butler_actions` row with `status='rejected'`, `rejection_reason='empty_evidence'`, `llm_usage_ledger_id=NULL` + `ButlerActionError(error_kind='empty_evidence')` before LLM gateway call (privacy gap: hallucinated plans on empty-citation context). Handler maps `'empty_evidence'` → `_MSG_EMPTY_EVIDENCE`. No migration (alembic head stays 078). No flag flip. | done | Branch `feat/p12-t12-09-binding`. Commits `3285039`..`2a598f8` (9 commits). Phase 11 binding **77 → 102** (77 prior + 25 new Butler ACs). 2-round dual-model review: Claude product ACCEPTED + Claude tech APPROVE (Codex companion stalled; used the documented independent-review fallback; 1 LOW handler routing fixed + regression test). Rollout: `docs/rollout-fragments/phase12/t12-09.md`. |
| T12-10 | 3 | Phase 12 closure sprint: FHR fix (scheduler savepoint per TTL expiry action — HIGH), ORM CHECK drift fix (`ButlerActionConfirmation.status` missing `'revoked'` — MEDIUM), closure docs (`PHASE12_ROLLOUT.md`, doc flips to CLOSED, `.env.example` ceiling vars). No migration (alembic head stays 078). No flag flip. FHR verdict: APPROVE. | done | Branch `feat/p12-t12-10-closure`. Commits `8dec11c` (scheduler savepoint, in-branch at start) + `e7f5188` (models.py CHECK drift) + closure-docs commit. Phase 11 binding **102/102**. Privacy lint exit 0. Ruff exit 0. Rollout: `docs/rollout-fragments/phase12/t12-10.md`. |

**Phase 12.5 carryovers:**
- Redact-at-rest for `extraction_candidates.candidate_json` + `butler_card_suggestions.suggested_card_payload` — pre-existing Phase-6 condition; read-side tombstone gates prevent promotion; deferred to Phase 6.5/12.5.
- Downgrade-guard hardening for migrations 075 (`'revoked'`) and 077 (`'undone'`) — unsafe downgrade while live rows hold those values; pre-flight guard (mirrors Phase 8 approach) deferred.
- Per-tool-flag service-layer defense-in-depth — handler-layer flags enforced; service-layer redundant check deferred.
- Callback master-flag re-check at callback time for long-running confirmations — deferred.
- T12-07: live-PG exercise of `_resolve_prior_text` JOIN + `_cascade_butler_undo_invocations` UPDATE — mock-only in T12-07; deferred to Phase 12.5.
- T12-09: I9.b auto-followup_correction on already-executed cited action edge case — deferred.
- T12-09: spec mask-format divergence (`[CONTENT_REDACTED: forget_event_id={n}]` spec vs shipped JSONB `{"redacted":true,"forget_event_id":n}`) — documented, privacy invariant holds, format normalisation deferred.
