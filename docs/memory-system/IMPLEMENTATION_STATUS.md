...
| T12-04 | 2 | `ButlerService` state machine (`plan_action`/`confirm_action`/`execute_action`/`cancel_action`/`expire_action`) + 6 exception classes + MSK rate buckets w/ rollback + cross-user consent re-verified at execute + cascade-vs-callback `SELECT FOR UPDATE NOWAIT` guard. Migration 074 adds `query`/`visibility_scope`/`plan_payload` to `butler_actions` + `confirmation_token`+UNIQUE on `butler_action_confirmations`. ORM + 4 repos extended. `forget_cascade._cascade_butler_action_confirmations` widened for parent-row lock. | PR pending | Branch `feat/p12-t12-04-statemachine`. Commits `3a09ecb`..`d70bb39` (incl. deep review fix cycle: 4 CRITICAL + 4 HIGH + class-identity + alembic head + rate-bucket rollback completeness). Alembic head 073 → 074. 58 state-machine + 33 repo + 9 cascade tests green (100 new). Phase 11 binding **86 → 86** (T12-04 adds no binding tests; L12/C10/I9/R8/G3 lands in T12-09). No flag flipped; foundation for T12-05 handlers + T12-06 tool wrappers + T12-07 undo + T12-08 reaper. |
