...
**Last updated:** 2026-05-27 (T12-06 docs gate — 5 butler tool implementations, round-2 wiring fixes, 136 tests, PR #349 pending merge.)
...
| T12-08 | 3 | Butler TTL expiry worker + per-user-day budget cap + rate bucket exhaustiveness + `monthly_cost_usd` call_type extension. `bot/services/butler_budget.py`: `ButlerBudgetChecker` (per-user-day ceiling via `BUTLER_PER_USER_DAILY_USD_CEILING` default $0.20) + `compute_user_daily_budget_spent` (JOIN `llm_usage_ledger ↔ butler_actions` for MSK-day SUM). `bot/services/scheduler.py`: `butler_expire_tick` inner function (iterates stale `pending_confirmation` rows via `_query_pending_past_ttl`, calls `ButlerService.expire_action` per action) + `butler_expire_tick_job` APScheduler wrapper (gated by `memory.butler.enabled`, `args=[bot]` F2 pattern, `BUTLER_EXPIRE_TICK_SECONDS` default 60s). `bot/db/repos/butler_action.py`: `get_pending_past_ttl` (SELECT pending_confirmation WHERE expires_at <= now). `bot/db/repos/llm_usage_ledger.py`: `monthly_cost_usd` gains `call_type` kwarg (C9 requirement, mirrors `daily_cost_usd` signature). No migration needed (budget tracked via existing `llm_usage_ledger ↔ butler_actions` join). | PR pending | Branch `feat/p12-t12-08-controls`. Commit `e8dc08f`. No new migration. 74 tests green (9 unit controls + 61 state-machine baseline + 4 scheduler; 6 skipped = DB-backed tests needing Postgres). Privacy lint exit 0. Phase 11 binding **86 → 86** (L12/C10/I9/R8/G3 lands in T12-09). No flag flipped. |
