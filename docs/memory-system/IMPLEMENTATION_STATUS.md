...
**Last updated:** 2026-05-29 (T12-09 — Phase 11 binding suite 77→102: 25 new
butler binding tests across L11/C10/I9/R8/G3 + R8.b abstain guard + G3.a AST
per-path scanner, all verified against live Postgres. PR #352.)
...
| T12-07 | 3 | Rollback + `/butler_undo <action_id>`. `ButlerService.undo_action` orchestrates inverse-op execution: acquires `get_for_update` (NOWAIT → `CascadeInFlightError`), authorizes (requester/admin-via-`user_repo`/affected_user) BEFORE revealing state, then writes a NEW child `butler_actions` row (`parent_action_id`→original, `status='undo_pending'`→`undo_succeeded`|`undo_failed`). Original row is **immutable** (never mutated). Child inherits `evidence_context_hash`+`evidence_ids`+`llm_usage_ledger_id` (C10.b citation preservation; satisfies `ck_butler_actions_ledger_required_post_plan`) and sets its own `rollback_kind='not_reversible'` (satisfies `ck_butler_actions_executed_has_inverse`). `_execute_inverse` dispatches on the original's `inverse_op_payload['rollback_kind']`: `delete_message`→`bot.delete_message` (chat=`chat_id`|`target_user_id`), `edit_message`→`bot.edit_message_text` to a neutral retraction notice (`parse_mode=None`; prior text never stored — privacy), `followup_correction`→`bot.send_message`, `cancel_pending`→`ExtractionCandidateRepo.mark_status('rejected')` **only if still pending** (FOR UPDATE; never clobbers an admin decision), `not_reversible`→no-op audit. Failure→`undo_failed`+content-free `error_context`→`ButlerUndoError`. Double-undo guard via new `ButlerActionRepo.list_children`. `create()` gains `parent_action_id`/`inverse_op_payload`/`undone_at`. Handler `/butler_undo` real (was stub): master-flag+membership gates, `session.rollback()` on guard early-returns (T12-05 middleware discipline), success vs not-reversible message keyed off child `result_payload`. | PR pending | Branch `claude/superflow-workflow-dev-XPvSB`. No migration (schema forward-provisioned in 070/074; alembic head unchanged at 076). 15 new service tests + 7 handler tests (327 butler-keyed pass, 116 skipped=Postgres). Privacy lint exit 0, ruff clean, no new mypy errors. Phase 11 binding **86 → 86** (delta 0; L12/C10/I9/R8/G3 lands in T12-09). No flag flipped. Adversarial review (1H+2M) addressed inline: rollback-on-early-return, cancel_pending still-pending guard, error_context DM chat_id. |
...
| T12-09 | 4 | Phase 11 binding suite expansion **86 → 102** (25 new butler binding tests across 5 families) + 1 implementation guard. **G3** (`tests/evals/test_butler_drift.py` + `test_no_llm_imports.py`): G3.a adds `assert_no_forbidden_imports_per_path(forbidden_map)` AST scanner (extends, not replaces, the global `LLM_PROVIDER_PREFIXES` scan) — forbids anthropic/openai/`bot.services.graph_query` in butler paths, empty-glob = harness misconfig, planted-violation meta-test; G3.b hash stability + card-source sensitivity; G3.c `ck_butler_actions_ledger_required_post_plan` enforced; G3.d `ck_butler_tool_invocations_tool_name` enforced. **R8** (`test_butler_refusal.py`): R8.a membership refusal (no evidence/LLM); **R8.b NEW guard** — `plan_action` refuses an abstained/empty-evidence bundle before the LLM call (rolls back rate buckets, writes rejected `empty_evidence` row, NULL ledger); R8.c cross-user consent; R8.d no graph_query import; R8.e TTL refusal; R8.f/g `validate_butler_plan` whitelist + args. **C10** (`test_butler_citations.py`): C10.a succeeded-row evidence resolves to live mv; C10.b undo child inherits `evidence_context_hash`+`evidence_ids`+ledger (DB-backed `undo_action`); C10.c `[^mv:N]`/`[^card:UUID]` token resolution vs redacted/archived. **I9** (`test_butler_forget_cascade.py`): I9.a-f assert the shipped T12-01 cascade layers (payload redaction, layer order, idempotency UNIQUE, pending→expired→fail-closed). **L11.c-e** (`test_butler_leakage.py`): forgotten/redacted source exclusion + affected-user preview surface. `scripts/lint_privacy_check.sh` allowlists the 4 new binding files. | PR pending (#352) | Branch `claude/superflow-workflow-dev-XPvSB`. No migration (alembic head 076). 25 binding tests (199 eval-suite pass) verified against live Postgres 16. Privacy lint exit 0, ruff clean. Reconciliation: I9.a/d assert the shipped `{"redacted":true,...}` whole-payload mask (not the spec's per-field `[CONTENT_REDACTED]` string — same privacy outcome); spec's distinct `source_card_forgotten` + auto followup_correction (I9.b) deferred to Phase 12.5. Phase 11 binding **86 → 102**. No flag flipped. |
