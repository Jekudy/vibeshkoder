...
| T12-05 | 2 | `bot/handlers/butler.py` — Telegram handlers: `/butler`, `/butler_status`, `/butler_cancel`, `/butler_undo` (stub) + 4 inline keyboard callbacks (confirm/cancel/affected_approve/affected_reject). DM-only (PrivateChatFilter), member auth gate, master feature flag `memory.butler.enabled` (default OFF) + 5 per-tool flags defined. Cross-user consent DM with Approve/Reject buttons. 8 §5.B rejection paths surfaced as polite Russian messages. Router registered in `bot/__main__.py`. | PR pending | Branch `feat/p12-t12-05-handlers`. Commit `95359c2`. No migrations (schema frozen at 074). 19 handler tests + 77/77 combined mode green. Privacy lint exit 0. Phase 11 binding **86 → 86** (T12-05 adds no binding tests; L12/C10/I9/R8/G3 lands in T12-09). |
