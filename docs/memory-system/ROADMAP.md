...
| 9 | Wiki (member / internal)                   | **AUTHORIZED 2026-05-17 (PHASE9_PLAN.md ratified)** | 8 sprints T9-01..T9-08; member/admin role split + governance validator + forget cascade integration (wiki_pages + wiki_revisions before card_sources) + ~18-21 new Phase 11 binding tests. Public wiki stays disabled by default. |
| 10| Graph projection (Neo4j / Graphiti)        | **AUTHORIZED 2026-05-17 (PHASE10_PLAN.md ratified)** | 9 sprints T10-01..T10-09; Neo4j 5.x via async cascade worker per RFC-001:415; replay-only full rebuild; 3 feature flags default OFF; ~15-16 new Phase 11 binding tests. |
...
Phase 8 closed 2026-05-15. **Phase 9 + Phase 10 ratified + authorized 2026-05-17** after dual-model spec review (Claude product + Codex technical) with 2 BLOCKER + 7-9 HIGH audit findings addressed across revision passes. Implementation waves T9-01..T9-08 and T10-01..T10-09 may dispatch in parallel where dependencies allow. Phase 8.5 carryovers (renderer polish, GIN index, #291 predicate, handler-layer R5 tightening) tracked in `IMPLEMENTATION_STATUS.md` and may be picked up opportunistically.
