...
| **A** | Phase 5 — Wave 2 (T5-03 repos) | T5-03 | 2026-05-11 | active | `feat/p5-w2-t5-03` (`.worktrees/p5-w2-t5-03`); PR TBD | Wave 2 SPLIT into 2 sequential PRs per `feedback-architectural-sprint-trigger` (privacy-critical isolation for T5-04). T5-03 scope: `bot/db/repos/llm_usage_ledger.py::LedgerRepo` (4 methods incl. `update_placeholder` per §12.2 REVISED) + `bot/db/repos/llm_synthesis_cache.py::SynthesisCacheRepo` (4 methods incl. `invalidate_by_citation` JSONB `@>`); ≥15 tests covering rollback, UTC boundary, JSONB containment, IntegrityError on duplicate `input_hash`. Flush-only, never commit. Protocol parity with `bot/services/llm_gateway.py::LedgerRepoProtocol` + `SynthesisCacheRepoProtocol`. Roles per `feedback-roles-claude-impl-codex-review`: `standard-implementer` (Sonnet, medium) + parallel `deep-code-reviewer` Claude critic + Codex tech review. |
| A | Phase 5 — Wave 2 (T5-04 handler + alembic 025 + cascade) | T5-04 | TBD (post-T5-03 merge) | queued | `feat/p5-w2-t5-04` (worktree TBD); PR TBD | **Privacy-critical** (invariant #2 + #3 + #9). Scope: alembic 025 extending `qa_traces` (4 new LLM cols + `DROP NOT NULL` on `prompt_hash`/`response_hash` per §12.1) + `QaTrace` ORM ext + `QaTraceRepo.update_llm_fields` (§12.3 binding sig) + `bot/services/llm_pricing.py` (§12.6 Haiku 4.5 + gpt-4o-mini) + `bot/handlers/qa.py` 4-step ORDER (§6.1: CREATE trace → synthesize → UPDATE fields → render) + flag `memory.qa.llm_synthesis.enabled` per-chat default FALSE + flag-OFF byte-for-byte Phase 4 preservation + `prompt_template_version` v0.1.0 → v1.0.0 (§12.5) + `bot/services/forget_cascade.py` 3 new layers in ORDER (`_cascade_llm_synthesis_cache` FIRST → `_cascade_qa_traces_llm` → `_cascade_llm_usage_ledger`). ≥20 tests. Roles: `deep-implementer` (Opus, high) + Claude critic + Codex tech review (cascade-coverage focus). |
...
**Last updated:** 2026-05-11 (Orchestrator A: Wave 2 kickoff — T5-03 active + T5-04 queued; Sprint 0 + Wave 0 + Wave 1 reconciled to merged status with PR refs).
