...
7. **Citation enforcement** — provider returns `ProviderResult.citation_ids: tuple[int, ...]` (explicit list — NOT post-parsed from answer text). Gateway validates `set(citation_ids) ⊆ set(surviving_ids)` (the POST-source-filter, POST-tombstone-gate set — NOT the pre-filter `bundle.evidence_ids` which could include offrecord/forgotten IDs). Hallucination OR citation to a filtered ID → ledger row `error='citation_hallucination'`, `Abstention(reason='provider_error')`. Closes Claude critic round-1 IMPORTANT-2 + Codex round-1 HIGH-3. Eval harness T5-05 verifies this property.
