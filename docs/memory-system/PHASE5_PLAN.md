...
class AnswerWithCitations:
    ...
    citation_ids: tuple[int, ...]  # subset of surviving_ids (post-source-filter)
    cost_usd: Decimal
    cache_hit: bool
...
1. **Empty bundle short-circuit** — if the input `EvidenceBundle` contains zero evidence IDs → write ledger row with `error='empty_bundle'`, return `Abstention(reason='empty_bundle', cost_usd=Decimal(0), llm_call_id=...)`. NO provider call.
...
7. **Citation enforcement** — provider returns `ProviderResult.citation_ids: tuple[int, ...]` (explicit list — NOT post-parsed from answer text). Gateway validates `set(citation_ids) ⊆ set(surviving_ids)` — the POST-source-filter, POST-tombstone-gate set. DO NOT compare against the original input bundle; offrecord/forgotten IDs may have been dropped at step 2 and citing them is privacy invariant #3 violation. Hallucination OR citation to a filtered ID → ledger row `error='citation_hallucination'`, `Abstention(reason='provider_error')`. Closes Claude critic round-1 IMPORTANT-2 + Codex round-1 HIGH-3. Eval harness T5-05 verifies this property.
...
|----|--------|-------|-------------|-----------|-----|-----------|--------------------|
| **T5-01** | A | `llm_gateway.py` core — `synthesize_answer`, provider abstraction (Anthropic+OpenAI), pre-call invariants, cache lookup | `bot/services/llm_gateway.py`, `bot/services/llm_providers/__init__.py`, `bot/services/llm_providers/anthropic.py`, `bot/services/llm_providers/openai.py`, `tests/services/test_llm_gateway.py`, `tests/services/test_llm_providers.py` | none (consumes 024) | T5-W0-01 merged + T5-02 schema (mock LedgerRepo until T5-03) | YES | All §5.A pre-call invariants enforced; provider errors abstain (no raise); citations subset of `surviving_ids` (post-source-filter set per §5.A step 7); cache hit writes ledger w/ `cache_hit=True`; cache invalidation when cited `message_version_id ∈ forget_events`; no LLM imports outside this file. ≥30 tests covering empty bundle, all-filtered, budget-exceeded, provider-error, forget-invalidated, cache-hit, citation-hallucination-rejection, source-filter race. |
...
6. Provider call returns content not in `surviving_ids` (post-source-filter set) — citation hallucination — gateway must reject AND eval suite must capture this regression.
...
- **`AnswerWithCitations`** — frozen dataclass returned on successful synthesis. `citation_ids ⊆ surviving_ids` (post-source-filter set per §5.A step 7).
